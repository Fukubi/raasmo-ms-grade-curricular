package com.rasmoo.client.escola.gradecurricular.config;
...
public class SwaggerConfig extends WebMvcConfigurationSupport {
	...
	public Docket gradeCurricularApi() {
		return new Docket(DocumentationType.SWAGGER_2).useDefaultResponseMessages(false).groupName("v1").select()
				.apis(RequestHandlerSelectors.basePackage("com.rasmoo.client.escola.gradecurricular.v1")).build()
	...
	private ApiInfo metaData() {
		return new ApiInfoBuilder().title("GRADE CURRICULAR")
				.description("API responsável pela manuntenção da grade curricular da rasmoo school").version("1.0.0")
				.license("").build();
	}
